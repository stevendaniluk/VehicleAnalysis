...
classdef Vehicle
    ...
    methods
        ...
        function this = setSpringPerchFromWheelLoads(this, x_fl, x_fr, x_rl, x_rr, FL, FR, RL, RR)
            % Subtract the sprung mass from each corner
            FL = FL - 0.5 * this.m_unsprung_f * this.gravity;
            FR = FR - 0.5 * this.m_unsprung_f * this.gravity;
            RL = RL - 0.5 * this.m_unsprung_r * this.gravity;
            RR = RR - 0.5 * this.m_unsprung_r * this.gravity;
        ...
        function [FL, FR, RL, RR] = longWeightTransferSuspension(this, varargin)
            ...
            % Default to full magnitude, then we'll scale it appropriately based on drive
            anti_f_perc = tan(this.SVIC_ang_f) * this.L ./ h_cog;
            anti_r_perc = tan(this.SVIC_ang_r) * this.L ./ h_cog;

            accelerating = a > 0;
            decelerating = a < 0;

            % Account for brake bias while decelerating
            anti_f_perc(decelerating) = this.brake_bias * anti_f_perc(decelerating);
            anti_r_perc(decelerating) = (1 - this.brake_bias) * anti_r_perc(decelerating);

            if ~this.FWD
                % No drive on front wheels, so zero out anti effects while accelerating
                anti_f_perc(accelerating) = 0;
            end
            if ~this.RWD
                % No drive on rear wheels, so zero out anti effects while accelerating
                anti_r_perc(accelerating) = 0;
            end
        ...
        function [FL, FR, RL, RR] = longWeightTransferGeometric(this, varargin)
            ...
            % Account for brake bias while decelerating
            anti_f_perc(decelerating) = this.brake_bias * anti_f_perc(a < 0);
            anti_r_perc(decelerating) = (1 - this.brake_bias) * anti_r_perc(a < 0);
        ...
        % longWeightTransferSprungTotal
        %
        % Computes the total amount of longitudinal weight transfer to the wheels due to
        % the sprung mass
        %
        % INPUTS:
        %   a: Longitudinal acceleration [m/s^2]
        ...
        %   RL: Rear left wheel load delta [N]
        %   RR: Rear right wheel load delta [N]
        function [FL, FR, RL, RR] = longWeightTransferSprungTotal(this, varargin)
            a = varargin{1};
            RH_r = varargin{2};
            RH_f = varargin{3};
        ...
        function [FL, FR, RL, RR] = totalWheelLoads(this, v, a, w, V_f, ...
            ...
            FL.geometric = FL_geo_lat + FL_geo_long;
            FR.geometric = FR_geo_lat + FR_geo_long;
            RL.geometric = RL_geo_lat + RL_geo_long;
            RR.geometric = RR_geo_lat + RR_geo_long;
        ...
        function [FL, FR, RL, RR] = estimatedAeroWheelLoad(this, a, V_f, ...
            ...
            %   F_shocks = F_static_sprung + F_wt - F_anti + F_aero
            %
            ...
            % Sum everything up
            FL.aero = FL.spring + FL.damper - FL.static - FL.anti;
            FR.aero = FR.spring + FR.damper - FR.static - FR.anti;
            RL.aero = RL.spring + RL.damper - RL.static - RL.anti;
            RR.aero = RR.spring + RR.damper - RR.static - RR.anti;
        end
